...
// CORRECTION 1 : On utilise un alias pour distinguer votre modèle de Flutter
...
class _RideRequestsScreenState extends State<RideRequestsScreen> with SingleTickerProviderStateMixin {
    ...
    return RefreshIndicator(
      ...
      child: Consumer<RideRequestProvider>(
        builder: (context, provider, _) {
          ...
          // Note: Assurez-vous que getActiveRequestsForDrivers existe dans votre provider
          // Si ça plante, remplacez par : provider.allRequests.where((r) => r.status == model.RideRequestStatus.active).toList()
          final activeRequests = provider.allRequests.where((r) => r.status == model.RideRequestStatus.active).toList();
    ...
    return RefreshIndicator(
      ...
      child: Consumer<RideRequestProvider>(
        builder: (context, provider, _) {
          ...
          final activeRequests = provider.getMyRequestsByStatus(model.RideRequestStatus.active);
    ...
    return Consumer<RideRequestProvider>(
      builder: (context, provider, _) {
        final historyRequests = provider.myRequests
            .where((req) => req.status != model.RideRequestStatus.active)
            .toList();
  ...
  // CORRECTION: request est de type model.RideRequest
  Widget _buildRequestCard(model.RideRequest request, {required bool isDriver, bool showProposals = false}) {
    return Card(
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: _buildInfoItem(
                        Icons.access_time,
                        // CORRECTION : Fonction helper pour afficher l'heure du modèle
                ...
                const SizedBox(height: 16),
                if (isDriver && request.status == model.RideRequestStatus.active)
                  SizedBox(
  ...
  // Helper pour afficher l'heure proprement sans conflit de type
  String _formatModelTime(dynamic time) {
    // On suppose que time a des propriétés hour et minute
  ...
  Widget _buildStatusChip(model.RideRequestStatus status) {
    Color color;
    String label;

    switch (status) {
      case model.RideRequestStatus.active:
        color = AppTheme.successGreen;
        label = 'Active';
        break;
      case model.RideRequestStatus.matched:
        color = AppTheme.primaryBlue;
        label = 'Acceptée';
        break;
      case model.RideRequestStatus.cancelled:
        color = AppTheme.errorRed;
        label = 'Annulée';
        break;
      case model.RideRequestStatus.expired:
        color = Colors.grey;
        label = 'Expirée';
        break;
    }
  ...
  void _showCreateRequestDialog() {
    ...
    // CORRECTION 2: Ici on utilise le TimeOfDay de FLUTTER pour l'UI
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  // On formate l'heure Flutter pour l'affichage
                  title: Text('${selectedTime.hour.toString().padLeft(2, '0')}:${selectedTime.minute.toString().padLeft(2, '0')}'),
                  onTap: () async {
                    // showTimePicker retourne un TimeOfDay de Flutter
                ...
                const SizedBox(height: 16),
                // ... (Le reste des widgets reste identique)
                Row(
                  children: [
                    const Icon(Icons.people),
                    const SizedBox(width: 16),
                    Expanded(
                      child: Text('Places: $seatsNeeded'),
                    ),
                    IconButton(
                      onPressed: seatsNeeded > 1
                          ? () => setState(() => seatsNeeded--)
                          : null,
                      icon: const Icon(Icons.remove_circle_outline),
                    ),
                    IconButton(
                      onPressed: seatsNeeded < 8
                          ? () => setState(() => seatsNeeded++)
                          : null,
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 16),
                    Expanded(
                      child: Text('Prix max: ${maxPrice.toStringAsFixed(0)} TND'),
                    ),
                    IconButton(
                      onPressed: maxPrice > 5
                          ? () => setState(() => maxPrice -= 5)
                          : null,
                      icon: const Icon(Icons.remove_circle_outline),
                    ),
                    IconButton(
                      onPressed: maxPrice < 100
                          ? () => setState(() => maxPrice += 5)
                          : null,
                      icon: const Icon(Icons.add_circle_outline),
                    ),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                // CORRECTION 3: CRUCIALE - CONVERSION FLUTTER -> MODELE
                // On utilise 'model.RideRequest' et 'model.TimeOfDay'
                final request = model.RideRequest(
  ...
  void _showProposalDialog(model.RideRequest request) {
        ...
        builder: (context, setState) => AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final user = context.read<AuthProvider>().currentUser;
                // Assurez-vous que RideProposal est bien exporté ou accessible
                // Si RideProposal est dans le même fichier que RideRequest, utilisez model.RideProposal
                final proposal = model.RideProposal(
  ...
  void _showProposals(model.RideRequest request) {
  ...
  Future<void> _acceptProposal(String requestId, String proposalId) async {
    try {
      ...
      if (context.mounted) Navigator.pop(context); // Fermer le bottom sheet
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
