...
class _RideDetailsScreenState extends State<RideDetailsScreen> {
  ...
  void _navigateToChat() async {
    ...
    if (user.id == widget.ride.driverId) {
      Navigator.pushNamed(context, AppRoutes.rideDiscussions, arguments: widget.ride.id);
      return;
    }

    final chatProvider = context.read<RideChatProvider>();
    final chatId = chatProvider.getChatId(
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    ...
                    child: widget.ride.driverImageUrl.isNotEmpty
                        ? ClipOval(child: Image.network(widget.ride.driverImageUrl, fit: BoxFit.cover, width: 80, height: 80))
                        : Text(
                            widget.ride.driverName.isNotEmpty ? widget.ride.driverName.substring(0, 1).toUpperCase() : 'D',
                            style: const TextStyle(
                              fontSize: 32,
                              fontWeight: FontWeight.bold,
                              color: AppTheme.primaryBlue,
                    ...
                    builder: (context, auth, _) {
                      return Stack(
                        children: [
                          ...
                          if (auth.currentUser?.id != widget.ride.driverId)
                            UnreadMessagesBadge(
                              rideId: widget.ride.id,
