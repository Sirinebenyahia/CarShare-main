...
class RideRequestService {
  ...
  Future<String> createRequest(RideRequest req) async {
    ...
    await doc.set(data);
    debugPrint('Request created successfully with ID: ${doc.id}');
  ...
  // ============================
  // GET MY REQUESTS (for MyRequestsScreen)
  // ============================
  Future<List<RideRequest>> getMyRequests(String passengerId) async {
    try {
      debugPrint('Fetching requests for passenger: $passengerId');
      ...
      final requests = snapshot.docs
          .map((doc) => requestFromJson(doc.data() as Map<String, dynamic>))
          .toList();
      
      // Sort locally instead of using orderBy to avoid index requirement
      requests.sort((a, b) => b.createdAt.compareTo(a.createdAt));
      
      debugPrint('Returning ${requests.length} requests');
